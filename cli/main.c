...
static void print_complete(int code, const char *title)
{
	const char *stamp = "???";
	switch (code) {
...
static void info_summary(summary_t *sum)
{
	int i, pass, total = 0;
	double proc;
...
static void info_msg(int code, void *ptr)
{
	switch (code) {
...
static void callback(int report, int code, void *ptr)
{
	result_t *res;
...
static void run(const char *file_name)
{
	ef_input(file_name);
}
